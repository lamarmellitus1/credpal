...
resource "aws_lb_target_group" "main" {
  name        = trimsuffix(substr("${var.service_name}-tg", 0, 32), "-")
  port        = 80
  protocol    = "HTTP"
  vpc_id      = var.vpc_id
  target_type = "ip"
...
resource "aws_appautoscaling_target" "ecs" {
  ...
  resource_id        = "service/${element(split("/", var.cluster_id), length(split("/", var.cluster_id)) - 1)}/${aws_ecs_service.main.name}"
...
resource "aws_appautoscaling_policy" "ecs_requests" {
  ...
  target_tracking_scaling_policy_configuration {
    predefined_metric_specification {
      ...
      resource_label         = "${var.alb_arn_suffix}/${aws_lb_target_group.main.arn_suffix}"
    }
